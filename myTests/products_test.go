...
func TestFindOneProduct(t *testing.T) {
	tests := []testFindOneProduct{
		...
		},
		{
			productId: "P000001",
			isErr:     false,
			expect:    `{"id":"P000001","title":"Coffee","description":"Just a food \u0026 beverage product","category":{"id":1,"title":"food \u0026 beverage"},"created_at":"2023-11-04T01:35:56.785433","updated_at":"2023-11-04T01:35:56.785433","price":150,"images":[{"id":"c580fe73-afb3-47d1-a9df-eed24fdaea9b","filename":"fb1_1.jpg","url":"https://i.pinimg.com/564x/4a/1c/4a/4a1c4a9755e4d3bdfcb45a1c3a58712f.jpg"},{"id":"43bcd3fa-6f7f-4251-b196-f30ad4ea625e","filename":"fb1_2.jpg","url":"https://i.pinimg.com/564x/4a/1c/4a/4a1c4a9755e4d3bdfcb45a1c3a58712f.jpg"},{"id":"77d9e690-b722-4039-b0fe-5f7d9af0e6b4","filename":"fb1_3.jpg","url":"https://i.pinimg.com/564x/4a/1c/4a/4a1c4a9755e4d3bdfcb45a1c3a58712f.jpg"}]}`,
		},
	}

	productModules := SetupTest().ProductsModule()
	for _, test := range tests {
		if test.isErr {
			if _, err := productModules.Usecase().FindOneProduct(test.productId); err.Error() != test.expect {
				t.Errorf("expect: %v, got %v", test.expect, err.Error())
			}
		} else {
			result, err := productModules.Usecase().FindOneProduct(test.productId)
			if err != nil {
